...
не принимается. Фактическое распределение после литературизации:

| Сезон | A (Automation) | B (Configuration/Tools) | C (Investigation) | D (Code) |
|---|---|---|---|---|
| Season 1 (14 серий) | 7 — e01, e06–e11 | 5 — e04, e05, e12, e13, e14 | 2 — e02, e03 | — |
| Season 2 (9 серий) | 4 — e01, e02, e03, e08 | 5 — e04–e07, e09 | — | — |

**Требование §2A выполнено:** ни один сезон не состоит из одного типа, Type B —
самая многочисленная категория в обоих.

**Открытый вопрос к автору (нужно решение).** Две серии Season 2 честнее было бы
пометить как Type C: `s02e02` (инвентаризация слушающих портов) и `s02e06` (аудит
правил фаервола) — по сути это расследования, а не автоматизация; план прямо
говорит, что bash-обёртки над `ss` стоит переводить в Type C. Смена метки требует
и смены артефакта — со скрипта на файл находок (как `recon.txt` в `s01e02`), то
есть переписывания задания и теста. Season 2 при этом останется без Type C только
формально: обе серии уже сейчас построены как наблюдение, а не как программирование.
