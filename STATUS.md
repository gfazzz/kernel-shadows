...
- **Воспроизводимость тестов.** ~~Требуют root и живого Linux.~~ Закрыто для S1–S2 (Этап 2): 23 теста на фикстурах и моках, корневой `Makefile`, CI с матрицей по сезонам, приёмкой на чистом клоне и прогонами под другой локалью. Для S3–S8 предстоит то же при их пересборке.
- **Перегрузка объёма.** Закрыто для S1–S2 (README 180–280 строк, `theory.md` 110–200). Осталось для S3–S8: README серий 1314–3620 строк, дробление 32 → ~63.
- **Нумерация.** `sNNeNN` в S1–S2 готово; S3–S8 ещё на схеме `episode-NN` (Этап 5–6).
...
- **Тонкие решения.** ~~ep24 (kernel), ep23~~ — закрыты пересборкой S6 (2026-08-21): `.ko` собирается и загружается, MQTT разобран и закрыт. Осталось: ep32 (капстоун).
