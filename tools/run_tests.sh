#!/usr/bin/env bash
#
# run_tests.sh — раннер unit-тестов всех серий курса.
#
...
#   REPEAT=2 bash tools/run_tests.sh             # два прогона (воспроизводимость)
...
# при каждом прогоне (лог должен переживать завершение раннера).
#
...
# ---- окружение прогона (в CI нужно знать, на чём гоняли) -------------
env_line() {
    printf 'bash %s | %s | %s\n' \
        "${BASH_VERSION%%(*}" \
        "$(uname -srm)" \
