#!/usr/bin/env bash
#
# log_analyzer.sh — s01e12 «Отчёт по атаке» (СТАРТЕР, капстоун ep03, Type B)
#
...

{
    ...
    #         Пропускай пустые строки и комментарии (#). Совпадение считай
    #         по ЦЕЛОМУ слову (grep -w), иначе 10.0.0.5 «найдётся» в 10.0.0.50.
    #         Печатай только найденные: "  FOUND: <ip> (<N> запросов)".
    echo "=== END OF REPORT ==="
} > "${report}"
