/* shadow_ring.c — перенесено из s06e11 БЕЗ ИЗМЕНЕНИЙ.
 *
 * Это уже проверенный код: менять его не надо, надо им пользоваться.
 * Так и выглядит накопление: каждая серия добавляет слой поверх
 * проверенного, а не переписывает его заново.
 *
