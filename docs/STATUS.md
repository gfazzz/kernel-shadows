...
|---|---|---|---|---|
| Season 1 (16 серий) | 6 — e01, e06–e09, e15 | 5 — e04, e05, e12–e14 | 5 — e02, e03, e10, e11, e16 | — |
| Season 2 (12 серий) | 3 — e01, e03, e08 | 5 — e05, e07, e09, e10, e11 | 4 — e02, e04, e06, e12 | — |
| **Итого (23)** | **8 (35 %)** | **8 (35 %)** | **7 (30 %)** | — |

Соотношение соответствует ориентирам для сезонов, где `.py` и C ещё не введены
...
| `s02e02` | A | **C** | `ports_report.txt` | обёртка над `ss`; такие обёртки правильнее переводить в Type C |
...
**Season 3 пересобран полностью** (2026-08-01): 4 эпизода → 13 серий, 281 проверка,
старые `episode-09..12` удалены после аудита потерь.

| Сезон | A — Automation | B — Configuration | C — Investigation | D — Code |
|---|---|---|---|---|
| Season 3 (13 серий) | 3 — e09, e11, e13 | 5 — e03, e05, e06, e10, e12 | 5 — e01, e02, e04, e07, e08 | — |
| Season 4 (12 серий) | 3 — e02, e08, e11 | 6 — e03, e05, e06, e07, e09, e12 | 3 — e01, e04, e10 | — |
| Season 5 (12 серий) | 3 — e01, e10, e12 | 5 — e03, e04, e06, e08, e11 | 4 — e02, e05, e07, e09 | — |

Соотношение 23/38/38 % — ровно то, чего требует от сезона про
администрирование: конфигурации больше, чем скриптов. Одно отклонение от
плана задокументировано в сезонном README: `s03e12` задумывалась вторым
`.timer`, отдана `logrotate` (таймеры разобраны в `s03e06`, а конфигурация
ротации — прямая причина находки `s03e08`).

Type D (программы на языке общего назначения) появляется начиная с Season 6.

**Season 5 пересобран полностью** (2026-08-09): 4 эпизода → 12 серий, 191 проверка,
старые `episode-17..20` вынесены в `personal/retired-s05/`. Сквозной артефакт —
одно расследование вторжения на `prod-web27` (то же, что нашли `s04e10`/`s04e11`):
снимки в `data/` (ss, access.log, auth.log, ps, /proc, lynis, AIDE) складываются
в хронологию `s05e10` и проверяемый базовый уровень `s05e12`. Баланс 3A/5B/4C —
для сезона про безопасность половина работы это чтение чужих следов, половина —
настройка защиты.

Наступательная сторона по договорённости с автором осталась **контекстом**:
инструменты атаки названы в теории, а зачётные задания разбирают её следы
(`access.log`, упавшая учётка, отчёт пентеста). Уязвимое приложение и нагрузки
Episode 18 сохранены как лаборатория `season-05/labs/web-sqli/`. Долг T4
(`fail2ban`) закрыт полностью в `s05e04`; долг T8 (`curl`) был закрыт при S4.
Сквозной метанавык сезона, названный явно: **система наблюдения бесполезна,
если наблюдаемый может её ослепить** (база AIDE офлайн, `-e 2` в auditd, сверка
`ps`↔`/proc`, enforce вместо complain), и **утверждение о безопасности полезно,
только если его можно опровергнуть** (финал — «X из Y по CIS», не «мы защищены»).

**Season 4 пересобран полностью** (2026-08-08): 4 эпизода → 12 серий, 278 проверок,
старые `episode-13..16` вынесены в `personal/retired-s04/` после аудита потерь.
Сквозной артефакт сезона — репозиторий `shadow_iac`: `s04e03` кладёт в него
`.gitignore`, `s04e05` — `Dockerfile`, `s04e06` — `compose.yaml`, `s04e07` —
конвейер, `s04e09` и `s04e12` — Ansible, а `s04e11` аудирует его целиком,
собирая полигон теста из эталонов и стартеров остальных серий сезона.

Аудит потерь дал два результата. Первый — новая серия: разбиение ep16 на
инвентарь, режим проверки и аудит учило Ansible **только на чтение**, поэтому
план изменён с 11 серий на 12 и добавлена `s04e12` (playbook, модули,
идемпотентность, handlers; заодно вернулся финал исходного эпизода —
«пятьдесят серверов одной командой»). Второй — раздел 7а в теории `s04e07`
(пирамида проверок, покрытие ≠ отсутствие ошибок). Оба отклонения
задокументированы в сезонном README. Монолиты покрыты полностью; мониторинг
(Prometheus/Grafana) сознательно оставлен Season 7 по THEORY_MAP, а разбор
конфликтов слияния уже был в теории `s04e02`.

**Season 2 достроен 2026-08-07: 9 → 12 серий.** Закрыты все три пробела, найденные
разбором недоразукрупнения S2: `s02e10` SSH-туннели (`~/.ssh/config`, 23/23), `s02e11` WireGuard
(`wg0.conf`, 19/19), `s02e12` журнал фаервола и `ufw limit` (`fwlog_report.txt`,
22/22, новый финал сезона). Двух серий из четырёх запланированных **не делал**:
разделение `s02e01` (подсети) и `s02e03` (маршрут) не подтвердилось измерением —
обе укладываются в бюджет объёма (211 и 207 строк README) и покрывают свои темы,
так что расщепление было бы добавлением объёма, а не закрытием пробела.
