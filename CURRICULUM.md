...
> **v2.0:** Season 7 пересобран в 12 атомарных серий (`s07e01`–`s07e12`, 426 проверок,
> 2A / 5B / 3C / 2D). Описания эпизодов ниже сохранены как карта тем и сюжета;
> актуальная разбивка — в [`season-07-production-advanced/README.md`](season-07-production-advanced/README.md).
> Сезон проходится **без кластера**: предмет проверки — манифесты как текст и снимки
> состояния, снятые заранее. Новая тема, которой в исходных эпизодах не было, —
> **SLO и бюджет ошибок** (`s07e12`); она же финал сезона.
...
**Итог Season 7:** Контур построен, измерен, ограничен в правах и описан числом.
Kubernetes, метрики, оповещения, разбор узкого места, SELinux, SLO. Исландия —
последний рубеж. Всё готово к финальной битве.

> **Про установку кластера, HPA, Grafana, нагрузочное тестирование и кэширование.**
> Отдельными сериями эти темы не стали: без живого кластера и живой нагрузки их
> нечем проверить, а тест, который делает вид, что проверил, хуже отсутствия теста.
> Каждая закрыта разделом теории — таблица соответствия в README сезона.
...
> **v2.0:** Season 8 пересобран в 12 атомарных серий (`s08e01`–`s08e12`, 355
> проверок, 3A/3B/3C/3D). Описания эпизодов ниже сохранены как карта тем и
> сюжета; актуальная разбивка — в [`season-08-final-operation/README.md`](season-08-final-operation/README.md).
> Финал переосмыслен как **приёмка** (`s08e12`): двенадцать фаз, по одной на
> навык сезона, каждая проверяет свойство и возвращает pass/fail. Капстоун
> `ep32` из 117 строк «в вакууме» заменён реальным прогоном.
...
**Итог Season 8:** Операция завершена приёмкой, которая возвращает ноль. Все 8
сезонов пересобраны в 101 атомарную серию, 60 дней, 8 стран. Макс Соколов:
от `pwd` до `shadow_core.py`. Главный урок финала — проверяемое утверждение
сильнее уверенного, а зелёная приёмка есть честная граница знания, а не
гарантия безопасности.

> **Про физическую конфронтацию и хвост PENUMBRA.** Встреча Крылов–Алекс в
> московском ЦОД осталась сюжетной рамкой: её нечем проверить тестом. Крючок
> к возможному продолжению (OPERATION PENUMBRA) оставлен в `s08e12`, но не
> развёрнут — курс учит инфраструктуре и заканчивается на проверяемом.
...
- ✅ Season 7: Production & Advanced (s07e01–s07e12, 426 проверок) — пересобран v2.0
- ✅ Season 8: Final Operation (s08e01–s08e12, 355 проверок) — пересобран v2.0
