#!/usr/bin/env bash
#
# s01e07 «Первый умный скрипт» — воспроизводимый unit-тест (без root, БЕЗ живой сети).
#
# Принцип mock-first: вместо реального ping в PATH подставляется мок-ping,
...
echo "════════════════════════════════════════════════════════════"
echo " s01e07 tests — subject: ${SCRIPT#"$SERIES_DIR"/}"
echo "════════════════════════════════════════════════════════════"

# Фикстура: мок-ping в отдельном bin, который кладём в начало PATH.
TEST_ROOT="$(mktemp -d 2>/dev/null || mktemp -d -t s01e07)"
