...
| Season 2 (9 серий) | 3 — e01, e03, e08 | 4 — e04, e05, e07, e09 | 2 — e02, e06 | — |

**Требование §2A выполнено:** ни один сезон не состоит из одного типа, в обоих
присутствуют все три задействованные категории.

**Решение автора (2026-08-01) выполнено:** `s02e02` и `s02e06` переведены в
**Type C**. Артефактом стал не скрипт, а заполненный отчёт о находках
(`ports_report.txt`, `fw_report.txt`) — по образцу `recon.txt` из `s01e02`.
Объекты разведки лежат в `season-02-networking/data/`: снимки `ss -tuln`,
`ufw status verbose`, список известных сервисов. Тесты сверяют находки студента
с этими снимками, вычисляя эталон командами, а не храня константы.
