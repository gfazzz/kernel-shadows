#!/usr/bin/env bash
#
# run_integration.sh — прогон тестов, которым нужен живой хост.
#
...
# проходят на unit-уровне без root и сети — включая те три, что план рефакторинга числил требующими живого хоста (s01e14, s02e06, s02e07): первая
...
if [ "${#tests[@]}" -eq 0 ]; then
    ...
    echo " и декларацией требований в mission.md."
    echo "════════════════════════════════════════════════════════════"
    exit 0
fi
