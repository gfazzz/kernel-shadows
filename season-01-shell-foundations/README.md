...
СЕРИЙ: 16 (s01e01–s01e16), 197 проверок, 14 ч 05 мин
```
...
**Season 1** — это введение в Ubuntu Linux и работу с терминалом. Ты научишься базовым командам, навигации, работе с файлами, text processing и package management. **Type B подход:** Linux tools first, bash second.
...
Season 1 пересобран в атомарные серии `sNNeNN` (см. CHANGELOG):
...
(mock-first); серии-разведки (`Type C`) сверяют находки студента с реальными
...
**Ориентация и файлы** (`s01e01`–`s01e06`): где ты находишься в дереве файловой
...
LILITH активируется в первой серии и сопровождает тебя через весь сезон:
...
### 2. Экспериментируй
Создай тестовую папку и пробуй команды:
```bash
mkdir ~/test_practice
cd ~/test_practice
...
### 5. Tab completion
Нажимай `Tab` для автодополнения имён файлов и команд.
...
К концу сезона у тебя есть шестнадцать зелёных тестов и, что важнее,
...
Создай issue в репозитории или напиши в discussions.
